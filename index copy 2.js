...
async function triggerSearch(page) {
  const ok = await page.evaluate(() => {
    if (typeof CSSM_SearchDataGrid === 'function') {
      CSSM_SearchDataGrid('QueryServiceCaseDg'); // 有些站函式名會變，下面備援會處理
      return true;
    }
    return false;
  }).catch(()=>false);
...
async function closeGridDialog(page) {
  const ok = await page.evaluate(() => {
    if (typeof CSSM_CloseDialog === 'function') {
      CSSM_CloseDialog(false, 'QueryServiceOrgDialog');
      return true;
    }
    return false;
  }).catch(()=>false);
  ...
  const btn = page.locator('button.btn.btn-danger[onclick*="CSSM_CloseDialog"][onclick*="QueryServiceOrgDialog"]');
  if (await btn.count()) {
...
function buildForm(base, overrides) {
  ...
  if (!f.has('nd')) f.set('nd', String(Date.now()));
  return f;
}
...
(async () => {
  ...
  /* ====== 硬重置：關閉彈窗 → 回主頁重開表單 → 重新抓 token / 建 API ====== */
  await closeGridDialog(page);
  await reopenFormFromHome(page);

  // 重抓 token（重開 Dialog 後可能改變）
  token = (await page.getAttribute('#QueryOrgServiceCaseForm input[name="__RequestVerificationToken"]', 'value')) || token;

  // 釋放舊 API，重建一個帶最新 cookie 的 API context
  await api.dispose().catch(()=>{});
  const api2 = await createApiFromPage(context, page);

  // 6) UI 操作「桃園=9、無名額=否」→ 抓第1頁 JSON（此時一定從第 1 頁開始）
  await page.selectOption('#county', '9');
  ...
  // 取得「無名額」第一頁的基準表單 + 用新的 API context 補頁
  const baseNoForm = new URLSearchParams(respNo1.request().postData() || '');
  const fetchNo = makeApiFetcher(api2, baseNoForm, token);
